...
async function incrementEpisodeViews(episodeId: string): Promise<void> {
  try {
    ...
    console.log('Episode view incremented successfully');
  } catch (error) {
    console.error('Database connection error:', error);
    throw error;
  }
...
async function getWebSeriesViews(contentId: string): Promise<number> {
  try {
    const { data, error } = await supabase.rpc('get_web_series_total_views', { web_series_content_id: contentId });
    if (error) {
      console.error('Error getting web series views:', error);
      return 0;
    }
    return data || 0;
  } catch (error) {
...
async function getContentViews(contentType: string, contentId: string): Promise<number> {
  try {
