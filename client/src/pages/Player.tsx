...
const Player = () => {
  ...
  // View tracking with 45-second threshold
  React.useEffect(() => {
    ...
    const trackView = () => {
      if (!hasTrackedView) {
        console.log('Tracking view after 45 seconds');
        // Track view when video starts playing
        if (contentType === 'movie' && contentId) {
          incrementMovieViews.mutate(contentId);
        } else if (episodeId) {
          if (contentType === 'show') {
            // For shows, increment show episode views
            incrementShowEpisodeViews.mutate(episodeId);
          } else {
            // For web series, increment regular episode views
    ...
    // Start view tracking timer when component mounts
    viewTimer = setTimeout(trackView, 45000); // 45 seconds
  ...
  }, [content, currentEpisode, id, incrementMovieViews, incrementEpisodeViews, incrementShowEpisodeViews, episodeId]);
